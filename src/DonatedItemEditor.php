...
function donatedItemEditorDbQuery(string $sql)
{
...
function donatedItemEditorDbFetchArray(&$result, ?int $mode = null)
{
    if ($mode === null) {
...
if ($iDonatedItemID > 0) {
    ...
    $rsDonatedItem = donatedItemEditorDbQuery($sSQL);
    $theDonatedItem = donatedItemEditorDbFetchArray($rsDonatedItem);
...
if ($iCurrentFundraiser) {
    ...
    $rsDeposit = donatedItemEditorDbQuery($sSQL);
    extract(donatedItemEditorDbFetchArray($rsDeposit));
...
if (isset($_POST['DonatedItemSubmit']) || isset($_POST['DonatedItemSubmitAndAdd'])) {
    ...
    if ($bGetKeyBack) {
        ...
        $rsDonatedItemID = donatedItemEditorDbQuery($sSQL);
        extract(donatedItemEditorDbFetchArray($rsDonatedItemID));
    }
    ...
    if ($iDonatedItemID > 0) {
        ...
        $rsDonatedItem = donatedItemEditorDbQuery($sSQL);
        extract(donatedItemEditorDbFetchArray($rsDonatedItem));
    ...
    <div class="card card-primary">
        <div class="card-body">
                    ...
                    <div class="col-md-4 col-md-offset-2 col-6">
                        ...
                        <div class="form-group">
                            ...
                            <select name="Donor" id="Donor" class="form-control select2">
                                ...
                                <?php
                                $rsPeople = donatedItemEditorDbQuery($sPeopleSQL);
                                while ($aRow = donatedItemEditorDbFetchArray($rsPeople)) {
                                    extract($aRow);
                        ...
                        <div class="form-group">
                            ...
                            <?php if ($bMultibuy) {
                                ...
                                <select name="Buyer" class="form-control">
                                    <option value="0" selected><?= gettext('Unassigned') ?></option>
                                <?php
                                $rsBuyers = donatedItemEditorDbQuery($sPaddleSQL);
                                while ($aRow = donatedItemEditorDbFetchArray($rsBuyers)) {
                                    extract($aRow);
            ...
            <div class="form-group text-center">
                ...
                <?php endif; ?>
                <input type="button" class="btn btn-secondary" value="<?= gettext('Cancel') ?>" name="DonatedItemCancel" onclick="document.location = '<?= RedirectUtils::escapeRedirectUrl($linkBack, 'v2/dashboard'); ?>';">
            </div>

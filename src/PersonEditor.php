...
function personEditorDbQuery(string $sql, bool $bStopOnError = true)
{
...
function personEditorDbFetchArray(&$result, ?int $mode = null)
{
    if ($mode === null) {
...
function personEditorDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function personEditorDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
$rsSecurityGrp = personEditorDbQuery($sSQL);

while ($aRow = personEditorDbFetchArray($rsSecurityGrp)) {
    extract($aRow);
...
$rsCustomFields = personEditorDbQuery($sSQL);
$numCustomFields = personEditorDbNumRows($rsCustomFields);
...
if (isset($_POST['PersonSubmit']) || isset($_POST['PersonSubmitAndAdd'])) {
    ...
    $aCustomData = [];
    while ($rowCustomField = personEditorDbFetchArray($rsCustomFields)) {
    ...
    if (!$bErrorFlag) {
        ...
        if ($numCustomFields > 0) {
            personEditorDbDataSeek($rsCustomFields, 0);
            $sSQL = '';
            while ($rowCustomField = personEditorDbFetchArray($rsCustomFields)) {
                extract($rowCustomField);
                if (AuthenticationManager::getCurrentUser()->isEnabledSecurity($aSecurityType[$custom_FieldSec])) {
            ...
            if ($sSQL > '') {
                if ($isSqlite) {
                    $ensureSQL = 'INSERT OR IGNORE INTO person_custom (per_ID) VALUES (' . $iPersonID . ')';
                    personEditorDbQuery($ensureSQL);
                    $updateSQL = 'UPDATE person_custom SET ' . $sSQL . ' per_ID = ' . $iPersonID;
                    personEditorDbQuery($updateSQL);
                } else {
                    $sSQL = 'REPLACE INTO person_custom SET ' . $sSQL . ' per_ID = ' . $iPersonID;
                    //Execute the SQL
                    personEditorDbQuery($sSQL);
                }
            }
        }
    ...
    if (!$isNewPerson) {
        ...
        $rsPerson = personEditorDbQuery($sSQL);
        extract(personEditorDbFetchArray($rsPerson));
        ...
        $rsCustomData = personEditorDbQuery($sSQL);
        $aCustomData = [];
        if (personEditorDbNumRows($rsCustomData) >= 1) {
            $aCustomData = personEditorDbFetchArray($rsCustomData);
        }
    }
}
...
$rsClassifications = personEditorDbQuery($sSQL);

//Get Families for the drop-down
$sSQL = 'SELECT * FROM family_fam ORDER BY fam_Name';
$rsFamilies = personEditorDbQuery($sSQL);

//Get Family Roles for the drop-down
$sSQL = 'SELECT * FROM list_lst WHERE lst_ID = 2 ORDER BY lst_OptionSequence';
$rsFamilyRoles = personEditorDbQuery($sSQL);
            ...
            <div class="row">
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="Family" id="familyId" class="form-control">
                        ...
                        <?php while ($aRow = personEditorDbFetchArray($rsFamilies)) {
                            extract($aRow);
                            $fam_ID = (int)$fam_ID;
                ...
                <div class="form-group col-md-3">
                    ...
                    <select name="FamilyRole" id="FamilyRole" class="form-control">
                        ...
                        <option value="" disabled>-----------------------</option>
                        <?php while ($aRow = personEditorDbFetchArray($rsFamilyRoles)) {
                            extract($aRow);
            ...
            <div class="row">
                <div class="form-group col-md-3">
                    ...
                    <select id="Classification" name="Classification" class="form-control">
                        <option value="0"><?= gettext('Unassigned') ?></option>
                        <option value="" disabled>-----------------------</option>
                        <?php while ($aRow = personEditorDbFetchArray($rsClassifications)) {
                            extract($aRow);
                            echo '<option value="' . $lst_OptionID . '"';

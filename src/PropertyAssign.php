...
function propertyAssignDbQuery(string $sql)
{
...
function propertyAssignDbNumRows($result): int
{
...
function propertyAssignDbFetchArray(&$result)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
if (isset($_GET['PersonID']) && AuthenticationManager::getCurrentUser()->isEditRecordsEnabled()) {
    ...
    $rsName = propertyAssignDbQuery($sSQL);
    $aRow = propertyAssignDbFetchArray($rsName);
    $sName = $aRow['grp_Name'];
...
function UpdateProperty($iRecordID, $sValue, $iPropertyID, $sAction)
{
    ...
    if ($sAction === 'add') {
        ...
        $rsExistingTest = propertyAssignDbQuery($sSQL);

        if (propertyAssignDbNumRows($rsExistingTest) === 0) {
            $sSQL = "INSERT INTO record2property_r2p (r2p_record_ID,r2p_pro_ID,r2p_Value) VALUES ($iRecordID,$iPropertyID,'$sValue')";
            propertyAssignDbQuery($sSQL);
        }
    } else {
...
$rsProperty = propertyAssignDbQuery($sSQL);
$aRow = propertyAssignDbFetchArray($rsProperty);
...
if ($sAction === 'edit') {
    ...
    $rsValue = propertyAssignDbQuery($sSQL);
    $aRow = propertyAssignDbFetchArray($rsValue);

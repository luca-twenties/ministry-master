...
function optionManagerDbQuery(string $sql, bool $bStopOnError = true)
{
...
function optionManagerDbFetchArray(&$result, ?int $mode = null)
{
...
function optionManagerDbNumRows($result): int
{
    if ($result === false || $result === true) {
        return 0;
    }
...
switch ($mode) {
    ...
    case 'grproles':
        ...
        $rsTemp = optionManagerDbQuery($sSQL);

        // Validate that this list ID is really for a group roles list. (for security)
        if (optionManagerDbNumRows($rsTemp) == 0) {
            RedirectUtils::redirect('v2/dashboard');
            break;
        }

        $aTemp = optionManagerDbFetchArray($rsTemp);
...
if (isset($_POST['AddField'])) {
    ...
    if (strlen($newFieldName) === 0) {
        ...
        $rsCount = optionManagerDbQuery($sSQL);
        if (optionManagerDbNumRows($rsCount) > 0) {
            $iNewNameError = 2;
        } else {
            // Get count of the options
            $sSQL = "SELECT '' FROM list_lst WHERE lst_ID = $listID";
            $rsTemp = optionManagerDbQuery($sSQL);
            $numRows = optionManagerDbNumRows($rsTemp);
            $newOptionSequence = $numRows + 1;

            // Get the new OptionID
            $sSQL = "SELECT COALESCE(MAX(lst_OptionID), 0) FROM list_lst WHERE lst_ID = $listID";
            $rsTemp = optionManagerDbQuery($sSQL);
            $aTemp = optionManagerDbFetchArray($rsTemp);
...
$rsList = optionManagerDbQuery($sSQL);
$numRows = optionManagerDbNumRows($rsList);
...
if (isset($_POST['SaveChanges'])) {
    for ($row = 1; $row <= $numRows; $row++) {
        $aRow = optionManagerDbFetchArray($rsList);
...
$sSQL = "SELECT lst_OptionName, lst_OptionID, lst_OptionSequence FROM list_lst WHERE lst_ID = $listID ORDER BY lst_OptionSequence";
$rsRows = optionManagerDbQuery($sSQL);
$numRows = optionManagerDbNumRows($rsRows);

// Create arrays of the option names and IDs
for ($row = 1; $row <= $numRows; $row++) {
    $aRow = optionManagerDbFetchArray($rsRows, defined('MYSQLI_BOTH') ? MYSQLI_BOTH : \PDO::FETCH_BOTH);

...
function groupViewDbQuery(string $sql)
{
...
function groupViewDbNumRows($result): int
{
...
function groupViewDbFetchArray(&$result)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function groupViewDbFetchRow(&$result): ?array
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        if ($result['index'] >= count($result['rows'])) {
            return null;
...
$rsEmailList = groupViewDbQuery($sSQL);
$sEmailLink = '';
$roleEmails = [];
$sMailtoDelimiter = AuthenticationManager::getCurrentUser()->getUserConfigString("sMailtoDelimiter");
while (($emailRow = groupViewDbFetchRow($rsEmailList)) !== null) {
    [$per_Email, $fam_Email, $virt_RoleName] = $emailRow;
    $sEmail = $per_Email;
    if ($sEmail) {
...
AND p2g2r_grp_ID = " . $iGroupID;
$rsPhoneList = groupViewDbQuery($sSQL);
$sPhoneLink = '';
$sCommaDelimiter = ', ';
while (($phoneRow = groupViewDbFetchRow($rsPhoneList)) !== null) {
    [$per_CellPhone] = $phoneRow;
    ...
    <div class="card-body">
        ...
        <div>
                    ...
                    if ($thisGroup->getHasSpecialProps()) {
                        ...
                        $aFieldFields = [];
                        $aTypeFields = [];
                        while ($aRow = groupViewDbFetchArray($rsPropList)) {
                            extract($aRow);
                    ...
                    if (groupViewDbNumRows($rsAssignedProperties) === 0) {
                        ...
                        ?>
                            <table class="table table-striped w-100">
                                ...
                                while ($aRow = groupViewDbFetchArray($rsAssignedProperties)) {
                                    extract($aRow);
                    ...
                    if (AuthenticationManager::getCurrentUser()->isManageGroupsEnabled()) {
                        ...
                        $availableProperties = [];
                        while ($aRow = groupViewDbFetchArray($rsProperties)) {
                            extract($aRow);

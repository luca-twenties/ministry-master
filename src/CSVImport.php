...
function csvImportDbQuery(string $sql, bool $bStopOnError = true)
{
    global $isSqlite, $dbConnection, $logger;
    ...
    try {
        ...
        $logger->error('SQLite query failed', ['sql' => $sql, 'exception' => $e]);
        if ($bStopOnError) {
            throw $e;
        }
        return false;
...
function csvImportDbFetchArray(&$result)
{
...
function csvImportDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
if (isset($_POST['UploadCSV'])) {
    ...
    if (empty($_FILES['CSVfile']['name'])) {
        ...
        $rsCustomFields = csvImportDbQuery($sSQL);

        $sPerCustomFieldList = '';
        while ($aRow = csvImportDbFetchArray($rsCustomFields)) {
        ...
        $rsfamCustomFields = csvImportDbQuery($sSQL);

        $sFamCustomFieldList = '';
        while ($aRow = csvImportDbFetchArray($rsfamCustomFields)) {
            if (!in_array($aRow['type_ID'], [9, 12])) {
        ...
        $rsSecurityGrp = csvImportDbQuery($sSQL);

        while ($aRow = csvImportDbFetchArray($rsSecurityGrp)) {
        ...
        $rsClassifications = csvImportDbQuery($sSQL); ?>
        <BR><BR>
        <select name="Classification">
            ...
            <?php
            while ($aRow = csvImportDbFetchArray($rsClassifications)) {
...
if (isset($_POST['DoImport'])) {
    ...
    if (is_file($csvTempFile)) {
        ...
        while ($aData = fgetcsv($pFile, 2048, ',')) {
                    ...
                    switch ($currentType) {
                        ...
                        case 7:
                            ...
                            if (empty($iEnv)) {
                                ...
                                $rsTemp = csvImportDbQuery($sSQL);
                                if (csvImportDbNumRows($rsTemp) === 0) {
                                    $iEnvelope = $iEnv;
                                } else {
                                    $iEnvelope = 0;
                                }
            ...
            csvImportDbQuery($sSQLperson);

            // Make a one-person family if requested
            if (isset($_POST['MakeFamilyRecords'])) {
                $sSQL = 'SELECT MAX(per_ID) AS iPersonID FROM person_per';
                $rsPersonID = csvImportDbQuery($sSQL);
                $aRow = csvImportDbFetchArray($rsPersonID);
                $iPersonID = $aRow['iPersonID'];
                $sSQL = 'SELECT * FROM person_per WHERE per_ID = ' . $iPersonID;
                $rsNewPerson = csvImportDbQuery($sSQL);
                extract(csvImportDbFetchArray($rsNewPerson));
                ...
                $rsExistingFamily = csvImportDbQuery($sSQL);
                $famid = 0;
                if (csvImportDbNumRows($rsExistingFamily) > 0) {
                    $aRow = csvImportDbFetchArray($rsExistingFamily);
                    $famid = $aRow['fam_ID'];
                ...
                if ($bHasFamCustom) {
                    ...
                    $rsFamCustomID = csvImportDbQuery($sSQL);
                    if (csvImportDbNumRows($rsFamCustomID) === 0) {
                        $familyCustom = new FamilyCustom();
                        $familyCustom->setFamId($famid);
                        $familyCustom->save();
                    }
                    ...
                    csvImportDbQuery($sSQLFamCustom);
                }
            }

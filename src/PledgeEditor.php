...
function pledgeEditorDbQuery(string $sql)
{
...
function pledgeEditorDbNumRows($result): int
{
...
function pledgeEditorDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function pledgeEditorDbFetchRow(&$result): ?array
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        if ($result['index'] >= count($result['rows'])) {
            return null;
...
function pledgeEditorDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
$rsFunds = pledgeEditorDbQuery($sSQL);
pledgeEditorDbDataSeek($rsFunds, 0);
while ($aRow = pledgeEditorDbFetchArray($rsFunds)) {
    $fun_ID = $aRow['fun_ID'];
...
if ($sGroupKey) {
    ...
    $rsKeys = pledgeEditorDbQuery($sSQL);
    while ($aRow = pledgeEditorDbFetchArray($rsKeys)) {
...
if (
    ...
    if (!$iMethod) {
        if ($sGroupKey) {
            ...
            $rsResults = pledgeEditorDbQuery($sSQL);
            $methodRow = pledgeEditorDbFetchRow($rsResults);
            $iMethod = $methodRow[0] ?? null;
            ...
            $rsMethod = pledgeEditorDbQuery($sSQL);
            $num = pledgeEditorDbNumRows($rsMethod);
            if ($num) {    // set iMethod to last record's setting
                $previousPledgeMethodArray = pledgeEditorDbFetchArray($rsMethod);
    ...
    if ($sGroupKey) {
        ...
        $rsResults = pledgeEditorDbQuery($sSQL);
        $resultRow = pledgeEditorDbFetchRow($rsResults);
        [
            $numGroupKeys,
            $PledgeOrPayment,
            $fundId,
            ...
            $iMethod,
            $iCurrentDeposit,
        ] = $resultRow ?? [null, null, null, null, null, null, null, null, null];

        $sSQL = "SELECT DISTINCT plg_famID, plg_CheckNo, plg_FYID from pledge_plg where plg_GroupKey='" . $sGroupKey . "'";
        $rsFam = pledgeEditorDbQuery($sSQL);
        $fam_NameArray = pledgeEditorDbFetchArray($rsFam);
        ...
        $rsAmounts = pledgeEditorDbQuery($sSQL);
        while ($aRow = pledgeEditorDbFetchArray($rsAmounts)) {
...
if ($PledgeOrPayment === 'Pledge') {
    ...
    $rsChecksThisDep = pledgeEditorDbQuery($sSQL);
    $depositCount = 0;
    while ($aRow = pledgeEditorDbFetchArray($rsChecksThisDep)) {
            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="col-lg-6">
                        ...
                        <?php $fundCount = count($fundId2Name); ?>
                        <input class="form-control" type="number" step="any" name="TotalAmount" id="TotalAmount" <?= $fundCount > 1 ? 'readonly' : '' ?> />
                        <?php if ($fundCount > 1) { ?>
                            <small class="text-muted"><?= gettext('Total is calculated from fund allocations. Edit the fund amounts below.') ?></small>
                        <?php } ?>
...
<script nonce="<?= SystemURLs::getCSPNonce() ?>">
    $(document).ready(function() {
        ...
        $("#TotalAmount").on("input", function() {
            // Only allow direct editing when there is a single fund.
            var fundInputs = $(".FundAmount");
            if (fundInputs.length === 1) {
                var totalValue = Number($(this).val());
                if (!Number.isNaN(totalValue)) {
                    fundInputs.val(totalValue);
                    CalculateTotal();
                }
            }
        });

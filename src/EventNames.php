...
function eventNamesDbQuery(string $sql)
{
...
function eventNamesDbNormalizeRow(array $row, ?int $mode)
{
    if ($mode === null) {
...
function eventNamesDbFetchArray(&$result, ?int $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        ...
        $result['index']++;
        return eventNamesDbNormalizeRow($row, $mode);
    }

    return mysqli_fetch_array($result, $mode);
}

function eventNamesDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
if (isset($_POST['Action'])) {
    switch (InputUtils::legacyFilterInput($_POST['Action'])) {
        case 'CREATE':
            ...
            $sSQL = $insert . $values;
            eventNamesDbQuery($sSQL);
            if ($isSqlite) {
                $theID = (int) $dbConnection->lastInsertId();
            } else {
            ...
            for ($j = 0; $j < $eCntNum; $j++) {
                ...
                if ($isSqlite) {
                    $sSQL = "INSERT OR IGNORE INTO eventcountnames_evctnm (evctnm_eventtypeid, evctnm_countname) VALUES ('" . InputUtils::legacyFilterInput($theID) . "','" . InputUtils::legacyFilterInput($cCnt) . "')";
                } else {
...
$rsOpps = eventNamesDbQuery($sSQL);
$numRows = eventNamesDbNumRows($rsOpps);

for ($row = 1; $row <= $numRows; $row++) {
    $aRow = eventNamesDbFetchArray($rsOpps);
    extract($aRow);
    ...
    $cOpps = eventNamesDbQuery($cSQL);
    $numCounts = eventNamesDbNumRows($cOpps);
    if ($numCounts) {
        $cCountName = [];
        for ($c = 1; $c <= $numCounts; $c++) {
            $cRow = eventNamesDbFetchArray($cOpps);
            extract($cRow);

...
function familyCustomFieldsDbQuery(string $sql)
{
...
function familyCustomFieldsDbNumRows($result): int
{
...
function familyCustomFieldsDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function familyCustomFieldsDbGetColumnStats(): array
{
    global $isSqlite, $dbConnection, $cnInfoCentral;

    if ($isSqlite) {
        $stmt = $dbConnection->query('PRAGMA table_info(family_custom)');
        $rows = $stmt ? $stmt->fetchAll(\PDO::FETCH_ASSOC) : [];
        if (count($rows) === 0) {
            return ['lastName' => null, 'lastIndex' => -1];
        }
        $last = $rows[count($rows) - 1];
        return ['lastName' => $last['name'] ?? null, 'lastIndex' => count($rows) - 1];
    }
    ...
    return ['lastName' => $fieldInfo ? $fieldInfo->name : null, 'lastIndex' => $last];
}
...
if (isset($_POST['SaveChanges'])) {
    ...
    $rsCustomFields = familyCustomFieldsDbQuery($sSQL);
    $numRows = familyCustomFieldsDbNumRows($rsCustomFields);

    for ($row = 1; $row <= $numRows; $row++) {
        $aRow = familyCustomFieldsDbFetchArray($rsCustomFields);
        extract($aRow);
    ...
    if (isset($_POST['AddField'])) {
        ...
        if (strlen($newFieldName) === 0) {
            ...
            $rsCustomNames = familyCustomFieldsDbQuery($sSQL);
            while ($aRow = familyCustomFieldsDbFetchArray($rsCustomNames)) {
            ...
            if (!$bDuplicateNameError) {
                ...
                $columnStats = familyCustomFieldsDbGetColumnStats();
                $lastFieldName = $columnStats['lastName'];
                $lastIndex = $columnStats['lastIndex'];
                ...
                if ($lastFieldName === null) {
                    ...
                    $newFieldNum = (int) mb_substr($lastFieldName, 1) + 1;
                    $newOrderID = $lastIndex + 1;
                }
                ...
                if ($newFieldType == 12) {
                    ...
                    $aTemp = familyCustomFieldsDbFetchArray(familyCustomFieldsDbQuery($sSQL));
                    if ($aTemp[0] > 9) {
                        $newListID = $aTemp[0] + 1;
                    } else {
                        $newListID = 10;
                ...
                familyCustomFieldsDbQuery($sSQL);
...
// Prepare Security Group list
    ...
    $rsSecurityGrp = familyCustomFieldsDbQuery($sSQL);

    $aSecurityGrp = [];
while ($aRow = familyCustomFieldsDbFetchArray($rsSecurityGrp)) {
    $aSecurityGrp[] = $aRow;
    extract($aRow);
...
<form method="post" action="FamilyCustomFieldsEditor.php" name="FamilyCustomFieldsEditor">
    ...
    ?>
        <div class="alert alert-info">
            ...
            <?= gettext('No custom Family fields have been added yet') ?>
        </div>
    <?php
    } else {
    ?>
            ...
            for ($row = 1; $row <= $numRows; $row++) {
                ...
                <tr>
                    ...
                    if ($aTypeFields[$row] == 9) {
                        ...
                        $rsGroupList = familyCustomFieldsDbQuery($sSQL);

                        while ($aRow = familyCustomFieldsDbFetchArray($rsGroupList)) {
                            extract($aRow);

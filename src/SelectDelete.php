...
function selectDeleteDbQuery(string $sql)
{
...
function selectDeleteDbNormalizeRow(array $row, ?int $mode)
{
    if ($mode === null) {
...
function selectDeleteDbFetchArray(&$result, ?int $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        ...
        $result['index']++;
        return selectDeleteDbNormalizeRow($row, $mode);
    }
...
function selectDeleteDbFetchRow(&$result)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        return selectDeleteDbFetchArray($result, defined('MYSQLI_NUM') ? MYSQLI_NUM : \PDO::FETCH_NUM);
    }

    return mysqli_fetch_row($result);
}

function selectDeleteDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
if (isset($_GET['Confirmed'])) {
    ...
    $rsProps = selectDeleteDbQuery($sSQL);

    while ($aRow = selectDeleteDbFetchRow($rsProps)) {
...
$rsFamily = selectDeleteDbQuery($sSQL);
extract(selectDeleteDbFetchArray($rsFamily));
...
<div class="card">
    <div class="card-body">
        ...
        $rsDonations = selectDeleteDbQuery($sSQL);
        $bIsDonor = (selectDeleteDbNumRows($rsDonations) > 0);
        ...
        if ($bIsDonor && !AuthenticationManager::getCurrentUser()->isFinanceEnabled()) {
            ...
            $rsFamilies = selectDeleteDbQuery($sSQL);
            ...
            $rs_head = selectDeleteDbQuery($sSQL);
            $aHead = '';
            while (list($head_firstname, $head_famid) = selectDeleteDbFetchRow($rs_head)) {
            ...
            while ($aRow = selectDeleteDbFetchArray($rsFamilies)) {
                extract($aRow);
            ...
            $rsPledges = selectDeleteDbQuery($sSQL); ?>
            <table class="table table-striped w-100">
                ...
                while ($aRow = selectDeleteDbFetchArray($rsPledges)) {
                    $tog = (!$tog);
                    $plg_FYID = '';
                    $plg_date = '';
                    $plg_amount = '';
            ...
            <?php
                ...
                $rsPerson = selectDeleteDbQuery($sSQL);
                while ($aRow = selectDeleteDbFetchArray($rsPerson)) {
                    extract($aRow);

...
function includeFunctionsDbQuery(string $sql)
{
...
function includeFunctionsDbFetchArray(&$result)
{
...
function includeFunctionsDbFetchRow(&$result): ?array
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        if ($result['index'] >= count($result['rows'])) {
            return null;
    ...
    // Handler for "person from group"
        case 9:
            if ($data > 0) {
                ...
                $rsTemp = includeFunctionsDbQuery($sSQL);
                extract(includeFunctionsDbFetchArray($rsTemp));
...
function formCustomField($type, string $fieldname, $data, ?string $special, bool $bFirstPassFlag): void
{
    ...
    // Handler for "person from group"
        case 9:
          ...
          // This allows the group special-property designer to allow selection from a specific group
            ...
            $rsGroupPeople = includeFunctionsDbQuery($sSQL);
            ...
            while ($aRow = includeFunctionsDbFetchArray($rsGroupPeople)) {
                extract($aRow);
    ...
    // Handler for custom lists
        case 12:
            ...
            $rsListOptions = includeFunctionsDbQuery($sSQL);
            ...
            while ($aRow = includeFunctionsDbFetchArray($rsListOptions)) {
                extract($aRow);
...
function FindMemberClassID()
{
    ...
    $rsClassifications = includeFunctionsDbQuery($sSQL);

    while ($aRow = includeFunctionsDbFetchArray($rsClassifications)) {
        extract($aRow);
...
function genGroupKey(string $methodSpecificID, string $famID, string $fundIDs, string $date)
{
    ...
    while (1) {
        ...
        $rsResults = includeFunctionsDbQuery($sSQL);
        $numGroupKeyRow = includeFunctionsDbFetchRow($rsResults);
        $numGroupKeys = $numGroupKeyRow[0] ?? null;
        if ($numGroupKeys) {
            ++$uniqueNum;
        } else {
            return $GroupKey;

...
-- MailChimp Plugin
INSERT OR IGNORE INTO config_cfg (cfg_id, cfg_name, cfg_value)
VALUES 
...
UPDATE config_cfg
SET cfg_value = '1'
WHERE cfg_name = 'plugin.mailchimp.enabled'
  AND COALESCE((SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sMailChimpApiKey'), '') != '';

UPDATE config_cfg
SET cfg_value = (SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sMailChimpApiKey')
WHERE cfg_name = 'plugin.mailchimp.apiKey'
  AND COALESCE((SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sMailChimpApiKey'), '') != '';
...
SET cfg_value = (
  ...
  WHERE cfg_name IN ('sNexmoAPIKey', 'sVonageAPIKey') AND cfg_value != ''
  LIMIT 1
)
WHERE cfg_name = 'plugin.vonage.apiKey'
  AND EXISTS (
    SELECT 1 FROM config_cfg
    WHERE cfg_name IN ('sNexmoAPIKey', 'sVonageAPIKey') AND cfg_value != ''
  );
...
SET cfg_value = (
  SELECT cfg_value FROM config_cfg
  WHERE cfg_name IN ('sNexmoAPISecret', 'sVonageAPISecret') AND cfg_value != ''
  LIMIT 1
)
WHERE cfg_name = 'plugin.vonage.apiSecret'
  AND EXISTS (
    SELECT 1 FROM config_cfg
    WHERE cfg_name IN ('sNexmoAPISecret', 'sVonageAPISecret') AND cfg_value != ''
  );
...
SET cfg_value = (
  SELECT cfg_value FROM config_cfg
  WHERE cfg_name IN ('sNexmoFromNumber', 'sVonageFromNumber') AND cfg_value != ''
  LIMIT 1
)
WHERE cfg_name = 'plugin.vonage.fromNumber'
  AND EXISTS (
    SELECT 1 FROM config_cfg
    WHERE cfg_name IN ('sNexmoFromNumber', 'sVonageFromNumber') AND cfg_value != ''
  );
...
UPDATE config_cfg
SET cfg_value = (SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sGoogleTrackingID')
WHERE cfg_name = 'plugin.google-analytics.trackingId'
  AND COALESCE((SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sGoogleTrackingID'), '') != '';
...
SET cfg_value = (
  SELECT cfg_value FROM config_cfg
  WHERE cfg_name = 'sOLPURL' AND cfg_value != '' AND cfg_value != 'http://192.168.1.1:4316'
  LIMIT 1
)
WHERE cfg_name = 'plugin.openlp.serverUrl'
  AND EXISTS (
    SELECT 1 FROM config_cfg
    WHERE cfg_name = 'sOLPURL' AND cfg_value != '' AND cfg_value != 'http://192.168.1.1:4316'
  );
...
UPDATE config_cfg
SET cfg_value = (SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sOLPUserName')
WHERE cfg_name = 'plugin.openlp.username'
  AND COALESCE((SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sOLPUserName'), '') != '';

-- Migrate OpenLP Password
UPDATE config_cfg
SET cfg_value = (SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sOLPPassword')
WHERE cfg_name = 'plugin.openlp.password'
  AND COALESCE((SELECT cfg_value FROM config_cfg WHERE cfg_name = 'sOLPPassword'), '') != '';
...
UPDATE config_cfg
SET cfg_value = COALESCE((SELECT cfg_value FROM config_cfg WHERE cfg_name = 'bEnableGravatarPhotos'), cfg_value)
WHERE cfg_name = 'plugin.gravatar.enabled';

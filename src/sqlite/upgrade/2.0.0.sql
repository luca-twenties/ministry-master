...
-- SQLite does not support DROP COLUMN without table rebuild. Safe to skip if column exists.

DELETE FROM config_cfg WHERE cfg_id IN (2, 4, 15, 17, 24, 32, 35, 999);

INSERT OR IGNORE INTO config_cfg (cfg_id, cfg_name, cfg_value, cfg_type, cfg_default, cfg_tooltip, cfg_section, cfg_category)
VALUES
  (2, 'debug', '1', 'boolean', '1',

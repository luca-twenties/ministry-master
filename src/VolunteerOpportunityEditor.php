...
function volunteerOpportunityEditorDbQuery(string $sql)
{
...
function volunteerOpportunityEditorDbNormalizeRow(array $row, ?int $mode)
{
    if ($mode === null) {
...
function volunteerOpportunityEditorDbFetchArray(&$result, ?int $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        ...
        $result['index']++;
        return volunteerOpportunityEditorDbNormalizeRow($row, $mode);
    }
...
function volunteerOpportunityEditorDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
if ($sAction === 'delete' && $iOpp > 0) {
    ...
    $rsOpps = volunteerOpportunityEditorDbQuery($sSQL);
    $aRow = volunteerOpportunityEditorDbFetchArray($rsOpps);
    extract($aRow);
        ...
        <div class="col-md-6">
                <div class="card border-danger">
                    ...
                    <div class="card-body">
                        ...
                        $rsPeople = volunteerOpportunityEditorDbQuery($sSQL);
                        $numRows = volunteerOpportunityEditorDbNumRows($rsPeople);
                        if ($numRows > 0) {
                            ...
                            for ($i = 0; $i < $numRows; $i++) {
                                $aRow = volunteerOpportunityEditorDbFetchArray($rsPeople);
                                extract($aRow);
...
if ($sAction === 'ConfDelete' && $iOpp > 0) {
    ...
    $rsOrder = volunteerOpportunityEditorDbQuery($sSQL);
    $aRow = volunteerOpportunityEditorDbFetchArray($rsOrder);
    $orderVal = $aRow[0];
    $sSQL = "DELETE FROM `volunteeropportunity_vol` WHERE `vol_ID` = '" . $iOpp . "'";
    volunteerOpportunityEditorDbQuery($sSQL);
...
if ($iRowNum === 0) {
    ...
    $rsOrder = volunteerOpportunityEditorDbQuery($sSQL);
    $numRows = volunteerOpportunityEditorDbNumRows($rsOrder);
    if ($numRows) {
        $sSQL = 'SELECT MAX(`vol_Order`) AS `Max_vol_Order` FROM `volunteeropportunity_vol`';
        $rsMax = volunteerOpportunityEditorDbQuery($sSQL);
        $aRow = volunteerOpportunityEditorDbFetchArray($rsMax);
        extract($aRow);
    ...
    $rsOpps = volunteerOpportunityEditorDbQuery($sSQL);
    $numRows = volunteerOpportunityEditorDbNumRows($rsOpps);

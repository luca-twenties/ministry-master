...
function userEditorDbQuery(string $sql)
{
...
function userEditorDbFetchArray(&$result)
{
...
function userEditorDbFetchRow(&$result)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        if ($result['index'] >= count($result['rows'])) {
            return false;
...
if (isset($_POST['save']) && $iPersonID > 0) {
    ...
    if ($iPersonID > 0) {
        ...
        $rsPeople = userEditorDbQuery($sSQL);
    }
}
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="PersonID" size="30" id="personSelect" class="form-control">
                                    ...
                                    while ($aRow = userEditorDbFetchArray($rsPeople)) {
                                        extract($aRow); ?>
    ...
    <div class="card-body box-danger">
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                $rsDefault = userEditorDbQuery($sSQL);
                $r = 1;
                // List Default Settings
                while ($aDefaultRow = userEditorDbFetchRow($rsDefault)) {
                    list(
                        $ucfg_per_id,
                        $ucfg_id,
                        $ucfg_name,
                    ...
                    $sSQL = "SELECT * FROM userconfig_ucfg WHERE ucfg_per_id='$usr_per_ID' "
                        . "AND ucfg_id='$ucfg_id' ";
                    $rsUser = userEditorDbQuery($sSQL);
                    while ($aUserRow = userEditorDbFetchRow($rsUser)) {
                        list(
                            $ucfg_per_id,
                            $ucfg_id,
                            $ucfg_name,

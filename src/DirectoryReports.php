...
function directoryReportsDbQuery(string $sql)
{
...
function directoryReportsDbNumRows($result): int
{
...
function directoryReportsDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function directoryReportsDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
$rsClassifications = directoryReportsDbQuery($sSQL);

//Get Family Roles for the drop-down
$sSQL = 'SELECT * FROM list_lst WHERE lst_ID = 2 ORDER BY lst_OptionSequence';
$rsFamilyRoles = directoryReportsDbQuery($sSQL);

// Get all the Groups
$sSQL = 'SELECT * FROM group_grp ORDER BY grp_Name';
$rsGroups = directoryReportsDbQuery($sSQL);

// Get the list of custom person fields
$sSQL = 'SELECT person_custom_master.* FROM person_custom_master ORDER BY custom_Order';
$rsCustomFields = directoryReportsDbQuery($sSQL);
$numCustomFields = directoryReportsDbNumRows($rsCustomFields);
...
$rsSecurityGrp = directoryReportsDbQuery($sSQL);

while ($aRow = directoryReportsDbFetchArray($rsSecurityGrp)) {
    extract($aRow);
...
<?php if (!array_key_exists('cartdir', $_GET)) {
    ...
    <tr>
        ...
        <td class="TextColumn">
            ...
            <?php
            while ($aRow = directoryReportsDbFetchArray($rsClassifications)) {
                extract($aRow);
    ...
    <tr>
        ...
        <td class="TextColumn">
            ...
            <select name="GroupID[]" size="5" multiple>
                <?php
                while ($aRow = directoryReportsDbFetchArray($rsGroups)) {
                    extract($aRow);
...
?>
    ...
    <tr>
        ...
        <td class="TextColumn">
            ...
            <?php
            while ($aRow = directoryReportsDbFetchArray($rsFamilyRoles)) {
                extract($aRow);
                echo '<option value="' . $lst_OptionID . '"';
    ...
    <tr>
        ...
        <td class="TextColumn">
            ...
            <?php
                directoryReportsDbDataSeek($rsFamilyRoles, 0);
            while ($aRow = directoryReportsDbFetchArray($rsFamilyRoles)) {
                extract($aRow);
                echo '<option value="' . $lst_OptionID . '"';
        ...
        <td class="TextColumn">
         ...
         <?php
            if ($numCustomFields > 0) {
                while ($rowCustomField = directoryReportsDbFetchArray($rsCustomFields)) {

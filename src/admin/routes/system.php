...
$app->group('/system', function (RouteCollectorProxy $group): void {
    ...
    $group->get('/doctor', function (Request $request, Response $response): Response {
        $renderer = new PhpRenderer(__DIR__ . '/../views/');

        $doctorReport = DoctorService::run();

        $pageArgs = [
            'sRootPath' => SystemURLs::getRootPath(),
            'sPageTitle' => gettext('System Doctor'),
            'doctor' => $doctorReport,
        ];

        return $renderer->render($response, 'doctor.php', $pageArgs);
    });

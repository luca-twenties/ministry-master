...
function pdfLabelDbQuery(string $sql)
{
...
function pdfLabelDbNumRows($result): int
{
...
function pdfLabelDbFetchArray(&$result)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function GroupBySalutation(string $famID, $aAdultRole, $aChildRole)
{
    ...
    $rsFamInfo = pdfLabelDbQuery($sSQL);

    if (pdfLabelDbNumRows($rsFamInfo) === 0) {
        return 'Invalid Family' . $famID;
    }

    $aFam = pdfLabelDbFetchArray($rsFamInfo);
    extract($aFam);
    ...
    $rsMembers = pdfLabelDbQuery($sSQL);
    $numMembers = pdfLabelDbNumRows($rsMembers);
    ...
    for ($ind = 0; $ind < $numMembers; $ind++) {
        $member = pdfLabelDbFetchArray($rsMembers);
        extract($member);
    ...
    if ($numAdult === 1) {
        ...
        $firstMember = pdfLabelDbFetchArray($rsMembers);
        extract($aAdult[0]);
        $firstFirstName = $per_FirstName;
        $firstLastName = $per_LastName;
        $secondMember = pdfLabelDbFetchArray($rsMembers);
        extract($aAdult[1]);
    ...
    if ($numChild > 2) {
        $thirdMember = pdfLabelDbFetchArray($rsMembers);
        extract($aChild[2]);
    ...
    if ($numChild > 3) {
        $fourthMember = pdfLabelDbFetchArray($rsMembers);
        extract($aChild[3]);
...
function GenerateLabels(&$pdf, $mode, $iBulkMailPresort, $bToParents, $bOnlyComplete): string
{
    ...
    $rsCartItems = pdfLabelDbQuery($sSQL);
    $didFam = [];

    while ($aRow = pdfLabelDbFetchArray($rsCartItems)) {

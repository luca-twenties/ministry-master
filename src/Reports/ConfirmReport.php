...
function confirmReportDbQuery(string $sql)
{
...
function confirmReportDbNumRows($result): int
{
...
function confirmReportDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function confirmReportDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
$rsCustomFields = confirmReportDbQuery($sSQL);
$numCustomFields = confirmReportDbNumRows($rsCustomFields);

if ($numCustomFields > 0) {
    $iFieldNum = 0;
    while ($rowCustomField = confirmReportDbFetchArray($rsCustomFields)) {
...
if ($_GET['familyId']) {
    ...
    foreach ($families as $family) {
        ...
        $rsFamilyMembers = confirmReportDbQuery($sSQL);
        $numFamilyMembers = 0;

        // Check if we have family members to display
        if (confirmReportDbNumRows($rsFamilyMembers) > 0) {
            ...
            confirmReportDbDataSeek($rsFamilyMembers, 0);
            $numFamilyMembers = 0;
            
            while ($aMember = confirmReportDbFetchArray($rsFamilyMembers)) {
                $numFamilyMembers++;
                extract($aMember);
        ...
        while ($aMember = confirmReportDbFetchArray($rsFamilyMembers)) {
            ...
            $sSQL = 'SELECT grp_ID, grp_Name, grp_hasSpecialProps, role.lst_OptionName AS roleName
                    ...
                    ORDER BY grp_Name';
            $rsAssignedGroups = confirmReportDbQuery($sSQL);
            
            if (confirmReportDbNumRows($rsAssignedGroups) > 0) {
                ...
                // Build groups string
                $groupsList = '';
                while ($aGroup = confirmReportDbFetchArray($rsAssignedGroups)) {
                    extract($aGroup);
                    if (!empty($groupsList)) {
                        $groupsList .= ', ';
                    }

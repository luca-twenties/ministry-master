...
function reminderReportDbQuery(string $sql)
{
...
function reminderReportDbNumRows($result): int
{
...
function reminderReportDbFetchArray(&$result)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function reminderReportDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
if (!empty($_POST['classList'])) {
    ...
    if ($classList[0]) {
        ...
        $rsClassifications = reminderReportDbQuery($sSQL);

        $inClassList = '(';
        $notInClassList = '(';

        while ($aRow = reminderReportDbFetchArray($rsClassifications)) {
            extract($aRow);
...
$rsFamilies = reminderReportDbQuery($sSQL);
...
if ($fundCount > 0) {
    ...
    for ($i = 0; $i < $fundCount; $i++) {
        ...
        $rsOneFund = reminderReportDbQuery($sSQL);
        $aFundName = reminderReportDbFetchArray($rsOneFund);
...
$rsFunds = reminderReportDbQuery($sSQL);
...
while ($aFam = reminderReportDbFetchArray($rsFamilies)) {
    extract($aFam);
    ...
    if ($pledge_filter === 'pledge') {
        ...
        $rsPledgeCheck = reminderReportDbQuery($temp);
        if (reminderReportDbNumRows($rsPledgeCheck) === 0) {
            continue;
        }
    }
    ...
    $rsPledges = reminderReportDbQuery($sSQL);

    // If there is no pledge or a payment go to next family
    if (reminderReportDbNumRows($rsPledges) === 0) {
        continue;
    }
    ...
    if ($only_owe === 'yes') {
        ...
        $oweByFund = [];
        $bOwe = 0;
        while ($aRow = reminderReportDbFetchArray($rsPledges)) {
            extract($aRow);
    ...
    if (reminderReportDbNumRows($rsFunds) > 0) {
        reminderReportDbDataSeek($rsFunds, 0);
        while ($row = reminderReportDbFetchArray($rsFunds)) {

...
function directoryReportDbQuery(string $sql)
{
...
function directoryReportDbNumRows($result): int
{
...
function directoryReportDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
    ...
    if ($mode === null) {
        $mode = defined('MYSQLI_ASSOC') ? MYSQLI_ASSOC : \PDO::FETCH_ASSOC;
    }

    return mysqli_fetch_array($result, $mode);
}

function directoryReportDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
function directoryReportGetServerVersion(): string
{
    global $dbConnection;

    if ($dbConnection instanceof \PDO) {
        $version = $dbConnection->getAttribute(\PDO::ATTR_SERVER_VERSION);
        return is_string($version) ? $version : '4.0';
    }
...
$rsCustomFields = directoryReportDbQuery($sSQL);
$numCustomFields = directoryReportDbNumRows($rsCustomFields);

if ($numCustomFields > 0) {
    while ($rowCustomField = directoryReportDbFetchArray($rsCustomFields)) {
        $pdf->addCustomField(
...
$mysqlinfo = directoryReportGetServerVersion();
...
if ($mysqlversion >= 4) {
    ...
    $rsRecords = directoryReportDbQuery($sSQL);
...
while ($aRow = directoryReportDbFetchArray($rsRecords)) {
    $OutStr = '';
    extract($aRow);
    ...
    if ($memberCount > 1) {
        ...
        $rsPerson = directoryReportDbQuery($sSQL);

        if (directoryReportDbNumRows($rsPerson) > 0) {
            $aHead = directoryReportDbFetchArray($rsPerson);
        ...
        if (directoryReportDbNumRows($rsPerson) > 0) {
            $aSpouse = directoryReportDbFetchArray($rsPerson);
        ...
        while ($aRow = directoryReportDbFetchArray($rsPerson)) {

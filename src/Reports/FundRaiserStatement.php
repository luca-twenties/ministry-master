...
function fundRaiserStatementDbQuery(string $sql)
{
...
function fundRaiserStatementDbFetchArray(&$result)
{
...
$rsPaddleNums = fundRaiserStatementDbQuery($sSQL);
...
while ($row = fundRaiserStatementDbFetchArray($rsPaddleNums)) {
    extract($row);
    ...
    if ($iPaddleNumID || isset($_POST["Chk$pn_ID"])) {
        ...
        $rsDonatedItems = fundRaiserStatementDbQuery($sSQL);
        ...
        while ($itemRow = fundRaiserStatementDbFetchArray($rsDonatedItems)) {
            extract($itemRow);
            $nextY = $curY;
            $pdf->SetXY(SystemConfig::getValue('leftX'), $curY);
        ...
        $rsPurchasedItems = fundRaiserStatementDbQuery($sSQL);
        ...
        while ($itemRow = fundRaiserStatementDbFetchArray($rsPurchasedItems)) {
            extract($itemRow);
            $nextY = $curY;
            $pdf->SetXY(SystemConfig::getValue('leftX'), $curY);
            $nextY = $pdf->cellWithWrap($curY, $nextY, $ItemWid, $tableCellY, $di_item, 0, 'L');
...
SQL;
        $rsMultiBuy = fundRaiserStatementDbQuery($sqlMultiBuy);
        while ($mbRow = fundRaiserStatementDbFetchArray($rsMultiBuy)) {

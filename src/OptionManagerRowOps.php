...
function optionManagerRowOpsDbQuery(string $sql)
{
...
function optionManagerRowOpsDbNumRows($result): int
{
...
function optionManagerRowOpsDbFetchArray(&$result)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
switch ($mode) {
    ...
    case 'grproles':
        ...
        $rsTemp = optionManagerRowOpsDbQuery($sSQL);
        if (optionManagerRowOpsDbNumRows($rsTemp) == 0) {
            RedirectUtils::redirect('v2/dashboard');
            break;
        }
...
switch ($sAction) {
    ...
    case 'up':
        ...
        optionManagerRowOpsDbQuery($sSQL);
    ...
    case 'delete':
        ...
        $rsPropList = optionManagerRowOpsDbQuery($sSQL);
        $numRows = optionManagerRowOpsDbNumRows($rsPropList);
        ...
        if ($numRows > 1) {
            ...
            if ($mode === 'grproles') {
                ...
                $rsTemp = optionManagerRowOpsDbQuery($sSQL);
                $aTemp = optionManagerRowOpsDbFetchArray($rsTemp);

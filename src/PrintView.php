...
function printViewDbQuery(string $sql)
{
...
function printViewDbFetchArray(&$result, ?int $mode = null)
{
...
function printViewDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function printViewDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
$rsPerson = printViewDbQuery($sSQL);
extract(printViewDbFetchArray($rsPerson));
...
$rsCustomFields = printViewDbQuery($sSQL);
$numCustomFields = printViewDbNumRows($rsCustomFields);

// Get the actual custom field data
$sSQL = 'SELECT * FROM person_custom WHERE per_ID = ' . $iPersonID;
$rsCustomData = printViewDbQuery($sSQL);
$aCustomData = printViewDbFetchArray($rsCustomData);
...
$sSQL = 'SELECT grp_ID, grp_Name, grp_hasSpecialProps, role.lst_OptionName AS roleName
        ...
        ORDER BY grp_Name';
$rsAssignedGroups = printViewDbQuery($sSQL);
...
$rsAssignedProperties = printViewDbQuery($sSQL);

// Get Field Security List Matrix
$sSQL = 'SELECT * FROM list_lst WHERE lst_ID = 5 ORDER BY lst_OptionSequence';
$rsSecurityGrp = printViewDbQuery($sSQL);

while ($aRow = printViewDbFetchArray($rsSecurityGrp)) {
    extract($aRow);
...
<table width="200">
    <tr>
        ...
        WHERE per_fam_ID = ' . $iFamilyID . ' ORDER BY fmr.lst_OptionSequence';
                    $rsFamilyMembers = printViewDbQuery($sSQL);
                }
                ?>
        ...
        <td width="33%" class="align-top" align="left">
            <table cellspacing="1" cellpadding="4">
                ...
                for ($i = 1; $i <= $numColumn1Fields; $i++) {
                    $Row = printViewDbFetchArray($rsCustomFields);
                    extract($Row);
...
?>
    ...
    <table class="table table-striped w-100">
        ...
        while ($aRow = printViewDbFetchArray($rsFamilyMembers)) {
            $per_BirthYear = '';
            $agr_Description = '';
    ...
    if (printViewDbNumRows($rsAssignedGroups) === 0) {
        ...
        while ($aRow = printViewDbFetchArray($rsAssignedGroups)) {
            extract($aRow);
            ...
            if ($grp_hasSpecialProps) {
                ...
                $rsPropList = printViewDbQuery($sSQL);

                $sSQL = 'SELECT * FROM groupprop_' . $grp_ID . ' WHERE per_ID = ' . $iPersonID;
                $rsPersonProps = printViewDbQuery($sSQL);
                $aPersonProps = printViewDbFetchArray($rsPersonProps);

                while ($aProps = printViewDbFetchArray($rsPropList)) {
                    extract($aProps);
    ...
    //Was anything returned?
    if (printViewDbNumRows($rsAssignedProperties) === 0) {
        ...
        while ($aRow = printViewDbFetchArray($rsAssignedProperties)) {
            $pro_Prompt = '';
            $r2p_Value = '';
            extract($aRow);
    ...
    if (AuthenticationManager::getCurrentUser()->isNotesEnabled()) {
        ...
        while ($aRow = printViewDbFetchArray($rsNotes)) {
            extract($aRow);

...
function eventEditorDbQuery(string $sql)
{
...
function eventEditorDbNormalizeRow(array $row, ?int $mode)
{
    if ($mode === null) {
...
function eventEditorDbFetchArray(&$result, ?int $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        ...
        $result['index']++;
        return eventEditorDbNormalizeRow($row, $mode);
    }

    return mysqli_fetch_array($result, $mode);
}

function eventEditorDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
if ($sAction === 'Create Event' && !empty($tyid)) {
    ...
    switch ($sDefRecurType) {
        ...
        case 'weekly':
            ...
            $ecOpps = eventEditorDbQuery($sSQL);
            $numRows = eventEditorDbNumRows($ecOpps);
            if ($numRows > 0) {
                // Use the most recent event if it exists
                $ecRow = eventEditorDbFetchArray($ecOpps);
                extract($ecRow);
    ...
    if (empty($_POST['EventTypeID'])) {
        ...
        $rsOpps = eventEditorDbQuery($sSQL);
        $aRow = eventEditorDbFetchArray($rsOpps);
        extract($aRow);
        $sTypeName = $type_name;
    }
    ...
    if ($iErrors === 0) {
        if ($EventExists === 0) {
            ...
            for ($c = 0; $c < $iNumCounts; $c++) {
                ...
                if ($isSqlite) {
                    $sSQL = "INSERT INTO eventcounts_evtcnt
                           ...
                           ('" . InputUtils::legacyFilterInput($iEventID) . "',
                            ...
                            '" . $filteredCount . "',
                            '" . $filteredCountNotes . "')
                           ON CONFLICT(evtcnt_eventid, evtcnt_countid) DO UPDATE SET
                             evtcnt_countcount=excluded.evtcnt_countcount,
                             evtcnt_notes=excluded.evtcnt_notes";
                } else {
                ...
                <tr>
                    ...
                    <td colspan="3" class="TextColumn">
                        <select name='EN_tyid' class='form-control w-100' id='event_type_id'>
                            ...
                            $rsEventTypes = eventEditorDbQuery($sSQL);
                            while ($aRow = eventEditorDbFetchArray($rsEventTypes)) {
                                extract($aRow);

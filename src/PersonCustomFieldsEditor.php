...
function personCustomFieldsDbQuery(string $sql)
{
...
function personCustomFieldsDbNumRows($result): int
{
...
function personCustomFieldsDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function personCustomFieldsDbGetColumnStats(): array
{
    global $isSqlite, $dbConnection, $cnInfoCentral;

    if ($isSqlite) {
        $stmt = $dbConnection->query('PRAGMA table_info(person_custom)');
...
<div class="card card-body">
    ...
    if (isset($_POST['SaveChanges'])) {
        ...
        $rsCustomFields = personCustomFieldsDbQuery($sSQL);
        $numRows = personCustomFieldsDbNumRows($rsCustomFields);

        for ($row = 1; $row <= $numRows; $row++) {
            $aRow = personCustomFieldsDbFetchArray($rsCustomFields);
            extract($aRow);
        ...
        if (isset($_POST['AddField'])) {
            ...
            if (strlen($newFieldName) === 0) {
                ...
                $rsCustomNames = personCustomFieldsDbQuery($sSQL);
                while ($aRow = personCustomFieldsDbFetchArray($rsCustomNames)) {
                ...
                if (!$bDuplicateNameError) {
                    ...
                    $columnStats = personCustomFieldsDbGetColumnStats();
                    ...
                    if ($newFieldType == 12) {
                        ...
                        $aTemp = personCustomFieldsDbFetchArray(personCustomFieldsDbQuery($sSQL));
                        if ($aTemp[0] > 9) {
                            $newListID = $aTemp[0] + 1;
                        } else {
                            $newListID = 10;
                    ...
                    personCustomFieldsDbQuery($sSQL);
    ...
    $rsSecurityGrp = personCustomFieldsDbQuery($sSQL);
    $aSecurityType = [];

    $aSecurityGrp = [];
    while ($aRow = personCustomFieldsDbFetchArray($rsSecurityGrp)) {
        $aSecurityGrp[] = $aRow;
        extract($aRow);
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if ($aTypeFields[$row] == 9) {
                                    ...
                                    $rsGroupList = personCustomFieldsDbQuery($sSQL);

                                    while ($aRow = personCustomFieldsDbFetchArray($rsGroupList)) {
                                        extract($aRow);

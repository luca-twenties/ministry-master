...
function getFamiliesWithAnniversaries(Request $request, Response $response, array $args): Response
{
    ...
    for ($i = -7; $i <= 7; $i++) {
        ...
        if ($isSqlite) {
            $monthExpr = "strftime('%m', " . FamilyTableMap::COL_FAM_WEDDINGDATE . ") = '" . str_pad((string) $month, 2, '0', STR_PAD_LEFT) . "'";
            $dayExpr = "strftime('%d', " . FamilyTableMap::COL_FAM_WEDDINGDATE . ") = '" . str_pad((string) $day, 2, '0', STR_PAD_LEFT) . "'";
        } else {
            $monthExpr = "MONTH(" . FamilyTableMap::COL_FAM_WEDDINGDATE . ") = {$month}";
            $dayExpr = "DAY(" . FamilyTableMap::COL_FAM_WEDDINGDATE . ") = {$day}";
        }
        $conditions[] = "({$monthExpr} AND {$dayExpr})";
    }

...
function peopleDashboardDbQuery(string $sql)
{
...
function peopleDashboardDbFetchRow(&$result)
{
...
$rsEmailList = peopleDashboardDbQuery($sSQL);
$sEmailLink = '';
$sMailtoDelimiter = AuthenticationManager::getCurrentUser()->getUserConfigString("sMailtoDelimiter");
$roleEmails = [];
while (list($per_Email, $fam_Email, $virt_RoleName) = peopleDashboardDbFetchRow($rsEmailList)) {
    $sEmail = $per_Email;
    if ($sEmail) {

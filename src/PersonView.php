...
function personViewDbQuery(string $sql)
{
...
function personViewDbNumRows($result): int
{
...
function personViewDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
$rsPerson = personViewDbQuery($sSQL);
extract(personViewDbFetchArray($rsPerson));
...
$rsCustomFields = personViewDbQuery($sSQL);

// Get the custom field data for this person.
$sSQL = 'SELECT * FROM person_custom WHERE per_ID = ' . $iPersonID;
$rsCustomData = personViewDbQuery($sSQL);
$aCustomData = personViewDbFetchArray($rsCustomData);
...
ORDER BY grp_Name';
$rsAssignedGroups = personViewDbQuery($sSQL);
$sAssignedGroups = ',';

// Get all the Groups
$sSQL = 'SELECT grp_ID, grp_Name FROM group_grp ORDER BY grp_Name';
$rsGroups = personViewDbQuery($sSQL);
...
$rsAssignedProperties = personViewDbQuery($sSQL);

// Get all the properties
$sSQL = "SELECT * FROM property_pro WHERE pro_Class = 'p' ORDER BY pro_Name";
$rsProperties = personViewDbQuery($sSQL);

// Get Field Security List Matrix
$sSQL = 'SELECT * FROM list_lst WHERE lst_ID = 5 ORDER BY lst_OptionSequence';
$rsSecurityGrp = personViewDbQuery($sSQL);

while ($aRow = personViewDbFetchArray($rsSecurityGrp)) {
    extract($aRow);
    ...
    <div class="col-lg-3 col-md-3 col-sm-3">
        ...
        <div class="card card-primary">
            ...
            <div class="card-body">
                ...
                $customFieldsHtml = '';
                while ($Row = personViewDbFetchArray($rsCustomFields)) {
                    extract($Row);
            ...
            <div class="card-body">
                <div class="tab-content">
                    ...
                    <div class="tab-pane" id="groups">
                        <div class="card">
                            <div class="card-body">
                                ...
                                if (personViewDbNumRows($rsAssignedGroups) === 0) {
                                ?>
                                    <br>
                                    ...
                                    while ($aRow = personViewDbFetchArray($rsAssignedGroups)) {
                                        extract($aRow); ?>
                                        <div class="col-md-4">
                                            <p><br /></p>
                                            <!-- Info box -->
                                            <div class="card card-info">
                                                ...
                                                if ($grp_hasSpecialProps) {
                                                    ...
                                                    $rsPropList = personViewDbQuery($sSQL);

                                                    $sSQL = 'SELECT * FROM groupprop_' . $grp_ID . ' WHERE per_ID = ' . $iPersonID;
                                                    $rsPersonProps = personViewDbQuery($sSQL);
                                                    $aPersonProps = personViewDbFetchArray($rsPersonProps);

                                                    echo '<div class="card-body">';

                                                    while ($aProps = personViewDbFetchArray($rsPropList)) {
                                                        extract($aProps);
                    ...
                    <div class="tab-pane" id="properties">
                        <?php
                        $sAssignedProperties = ','; ?>
                        <?php if (personViewDbNumRows($rsAssignedProperties) === 0) : ?>
                            <div class="alert alert-warning">
                        ...
                        <?php else : ?>
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    <?php
                                    while ($aRow = personViewDbFetchArray($rsAssignedProperties)) {
                                        $pro_Prompt = '';
                                        $r2p_Value = '';
                                        extract($aRow); ?>
                                        <tr>
                        ...
                        <?php if ($bOkToEdit && personViewDbNumRows($rsProperties) !== 0) : ?>
                            <div class="card mt-3">
                                <div class="card-header">
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <select name="PropertyId" id="input-person-properties" class="form-control select2" data-placeholder="<?= gettext('Choose a property...') ?>">
                                                    <option value=""></option>
                                                            ...
                                                            while ($aRow = personViewDbFetchArray($rsProperties)) {
                                                                extract($aRow);
                    ...
                    <div class="tab-pane" id="volunteer">
                        ...
                        if (personViewDbNumRows($rsAssignedVolunteerOpps) === 0) {
                        ?>
                            <div class="alert alert-warning">
                        ...
                        <?php
                            ...
                            while ($aRow = personViewDbFetchArray($rsAssignedVolunteerOpps)) {
                                extract($aRow);
                                echo '<tr>';
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-6">
                                                ...
                                                <select id="input-volunteer-opportunities" name="VolunteerOpportunityIDs[]" multiple class="form-control select2" data-placeholder="<?= gettext('Choose opportunities...') ?>">
                                                    <?php
                                                    while ($aRow = personViewDbFetchArray($rsVolunteerOpps)) {
                                                        extract($aRow);

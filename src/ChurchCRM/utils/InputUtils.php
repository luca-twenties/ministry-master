...
class InputUtils
{
    ...
    public static function legacyFilterInput($sInput, $type = 'string', $size = 1)
    {
        ...
        if (strlen($sInput) > 0) {
            ...
            switch ($type) {
                case 'string':
                    $value = self::sanitizeText($sInput);
                    return $isSqlite && class_exists('\\SQLite3')
                        ? \SQLite3::escapeString($value)
                        : mysqli_real_escape_string($cnInfoCentral, $value);
                case 'htmltext':
                    $value = self::sanitizeHTML($sInput);
                    return $isSqlite && class_exists('\\SQLite3')
                        ? \SQLite3::escapeString($value)
                        : mysqli_real_escape_string($cnInfoCentral, $value);
                case 'char':
                    $value = self::filterChar($sInput, $size);
                    return $isSqlite && class_exists('\\SQLite3')
                        ? \SQLite3::escapeString($value)
                        : mysqli_real_escape_string($cnInfoCentral, $value);
                case 'int':

...
class SQLUtils
{
     ...
     */
    public static function sqlImport(string $fileName, $connection): void
    {
        $delimiter = ';';
    ...
    private static function query(string $sql, $connection): void
    {
        ...
        if ($connection instanceof \PDO) {
            ...
            if ($result === false) {
                ...
                throw new \Exception("Cannot execute request to the database {$sql}: " . $message);
            }
            return;
        }

        if (!$query = $connection->query($sql)) {
            $error = property_exists($connection, 'error') ? $connection->error : 'unknown error';
            throw new \Exception("Cannot execute request to the database {$sql}: " . $error);
        }
    }
}

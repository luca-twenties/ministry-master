...
class GroupService
{
    ...
    private function dbNumRows($result): int
    {
        if (is_array($result) && array_key_exists('rows', $result)) {
    ...
    private function addPersonToGroupProperties(int $groupID, int $personID): void
    {
        ...
        $sql = $this->isSqlite()
            ? 'INSERT OR IGNORE INTO groupprop_' . (int)$groupID . ' (per_ID) VALUES (:personId)'
            : 'INSERT IGNORE INTO groupprop_' . (int)$groupID . ' (per_ID) VALUES (:personId)';
    ...
    public function removeUserFromGroup(int $groupID, int $personID): void
    {
        ...
        $rsTemp = $this->dbQuery($sSQL);
        $rowTemp = $this->dbFetchRow($rsTemp);
        $bHasProp = $rowTemp[0];
        ...
        $result = $this->dbQuery($sSQL);
        while ($aRow = $this->dbFetchArray($result)) {
    ...
    public function getGroupRoles(int $groupID): array
    {
        ...
        if ($this->dbNumRows($rsList) === 0) {
            throw new \Exception('invalid request');
        }

        while ($row = $this->dbFetchAssoc($rsList)) {
            $groupRoles[] = $row;
        }
    ...
    public function getGroupRoleOrder(string $groupID, string $groupRoleID)
    {
        ...
        $rowOrder = $this->dbFetchArray($rsPropList);
    ...
    public function deleteGroupRole(string $groupID, string $groupRoleID): array
    {
        ...
        $sSQL = 'SELECT * FROM list_lst
                INNER JOIN group_grp
                 ...
                 WHERE group_grp.grp_ID = "' . $groupID . '"';
        $rsPropList = FunctionsUtils::runQuery($sSQL);
        $numRows = $this->dbNumRows($rsPropList);
        ...
        if ($numRows > 1) {
            ...
            $aTemp = $this->dbFetchArray($rsTemp);
    ...
    public function enableGroupSpecificProperties(string $groupID): void
    {
        ...
        foreach ($groupMembers as $member) {
            if ($isSqlite) {
                $sSQLr = 'INSERT OR IGNORE INTO groupprop_' . $groupID . " ( per_ID ) VALUES ( '" . $member['per_ID'] . "' );";
            } else {

...
class FinancialService
{
    ...
    {
        $dbDriver = getenv('CHURCHCRM_DB_DRIVER') ?: ($GLOBALS['sDATABASEDriver'] ?? 'mysql');
        return strtolower((string) $dbDriver) === 'sqlite';
    }

    private function dbQuery(string $sql)
    {
        if (!$this->isSqlite()) {
            return FunctionsUtils::runQuery($sql);
        }
    ...
    private function dbFetchArray(&$result)
    {
    ...
    private function dbFetchRow(&$result): ?array
    {
        if (is_array($result) && array_key_exists('rows', $result)) {
            if ($result['index'] >= count($result['rows'])) {
                return null;
    ...
    private function dbDataSeek(&$result, int $offset): void
    {
        if (is_array($result) && array_key_exists('rows', $result)) {
            $result['index'] = $offset;
            return;
    ...
    public function getMemberByScanString(string $tScanString): array
    {
        ...
        $rsFam = $this->dbQuery($sSQL);
        $row = $this->dbFetchArray($rsFam);
    ...
    public function getDepositTotal($id, $type = null)
    {
        ...
        $rsDepositTotal = $this->dbQuery($sSQL);
        $depositRow = $this->dbFetchRow($rsDepositTotal);
        $deposit_total = $depositRow[0] ?? null;
    ...
    public function locateFamilyCheck(string $checkNumber, string $fam_ID)
    {
        ...
        $sSQL = 'SELECT count(plg_FamID) from pledge_plg
                 ...
                 plg_FamID = ' . $fam_ID;
        $rCount = $this->dbQuery($sSQL);
        $countRow = $this->dbFetchArray($rCount);

        return $countRow[0] ?? null;
    }
    ...
    public function getPledgeorPayment(string $GroupKey): string
    {
        ...
        $rsKeys = $this->dbQuery($sSQL);
        $payment = new \stdClass();
        $payment->funds = [];
        while ($aRow = $this->dbFetchArray($rsKeys)) {
            extract($aRow);
    ...
    public function getCurrencyTypeOnDeposit(string $currencyID, string $depositID)
    {
        ...
        $rscurrencyDenomination = $this->dbQuery($sSQL);

        $denominationRow = $this->dbFetchArray($rscurrencyDenomination);

        return $denominationRow[0] ?? null;
    }
    ...
    public function getCurrency(): array
    {
        ...
        $rscurrencyDenomination = $this->dbQuery($sSQL);
        $this->dbDataSeek($rscurrencyDenomination, 0);
        while ($row = $this->dbFetchArray($rscurrencyDenomination)) {
    ...
    public function getActiveFunds(): array
    {
        ...
        $rsFunds = $this->dbQuery($sSQL);
        $this->dbDataSeek($rsFunds, 0);
        while ($aRow = $this->dbFetchArray($rsFunds)) {
            $fund = new \stdClass();
            $fund->ID = $aRow['fun_ID'];

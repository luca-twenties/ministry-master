...
class FinancePaymentSearchResultProvider extends BaseSearchResultProvider
{
    ...
    private function getPaymentsWithValuesInRange(int $min, int $max): array
    {
        ...
        try {
            ...
            ->withColumn(
                $isSqlite
                    ? "'#' || " . PledgeTableMap::COL_PLG_PLGID
                    : 'CONCAT("#",' . PledgeTableMap::COL_PLG_PLGID . ')',
                'displayName'
            )
            ->withColumn(
                $isSqlite
                    ? "'" . SystemURLs::getRootPath() . "/DepositSlipEditor.php?DepositSlipID=' || " . PledgeTableMap::COL_PLG_DEPID
                    : 'CONCAT("' . SystemURLs::getRootPath() . '/DepositSlipEditor.php?DepositSlipID=",' . PledgeTableMap::COL_PLG_DEPID . ')',
                'uri'
            )

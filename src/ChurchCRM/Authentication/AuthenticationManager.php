...
class AuthenticationManager
{
    ...
    public static function getAuthenticationProvider(): IAuthenticationProvider
    {
        ...
        if (AppMode::isStandalone() && isset($_SESSION['authUserId'])) {
            try {
                $user = UserQuery::create()->findPk((int) $_SESSION['authUserId']);
                if ($user instanceof User) {
                    $provider = LocalAuthentication::fromSession(
                        $user,
                        isset($_SESSION['authLastOp']) ? (int) $_SESSION['authLastOp'] : null
                    );
                    self::setAuthenticationProvider($provider);
                    return $provider;
                }
            } catch (\Throwable $e) {
                LoggerUtils::getAuthLogger()->debug('Failed to restore standalone session provider', ['exception' => $e]);
            }
        }

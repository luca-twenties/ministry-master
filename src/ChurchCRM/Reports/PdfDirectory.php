...
function pdfDirectoryDbQuery(string $sql)
{
...
function pdfDirectoryDbNumRows($result): int
{
...
function pdfDirectoryDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function pdfDirectoryDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
//        $this->SetY($_PosY + $this->nbLines($this->_ColWidth, $sName) * 5);
    ...
    {
        $numCustomFields = pdfDirectoryDbNumRows($rsCustomFields);
        if ($numCustomFields > 0) {
            extract($aRow);
            $sSQL = 'SELECT * FROM person_custom WHERE per_ID = ' . $per_ID;
            $rsCustomData = pdfDirectoryDbQuery($sSQL);
            $aCustomData = pdfDirectoryDbFetchArray($rsCustomData);
            $numCustomData = pdfDirectoryDbNumRows($rsCustomData);
            pdfDirectoryDbDataSeek($rsCustomFields, 0);
            $OutStr = '';
            while ($rowCustomField = pdfDirectoryDbFetchArray($rsCustomFields)) {

...
function csvCreateFileDbQuery(string $sql)
{
...
function csvCreateFileDbNumRows($result): int
{
...
function csvCreateFileDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function csvCreateFileDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
if ($sFormat === 'default') {
    ...
    $rsCustomFields = csvCreateFileDbQuery($sSQL);

    $sSQL = 'SELECT * FROM family_custom_master ORDER BY fam_custom_Order';
    $rsFamCustomFields = csvCreateFileDbQuery($sSQL);
}
if ($sFormat === 'rollup') {
    $sSQL = 'SELECT * FROM family_custom_master ORDER BY fam_custom_Order';
    $rsFamCustomFields = csvCreateFileDbQuery($sSQL);
...
$rsFamilyRoles = csvCreateFileDbQuery($sSQL);
while ($aRow = csvCreateFileDbFetchArray($rsFamilyRoles)) {
    extract($aRow);
...
$rsSecurityGrp = csvCreateFileDbQuery($sSQL);

while ($aRow = csvCreateFileDbFetchArray($rsSecurityGrp)) {
    extract($aRow);
...
if ($sSource === 'cart') {
    ...
    if (!empty($_POST['BirthDate1'])) {
        if ($isSqlite) {
            $sWhereExt .= "AND date(printf('%04d-%02d-%02d', per_BirthYear, per_BirthMonth, per_BirthDay)) >= '" . InputUtils::legacyFilterInput($_POST['BirthDate1'], 'char', 10) . "' ";
        } else {
    ...
    if ($_POST['BirthDate2'] != date('Y-m-d')) {
        if ($isSqlite) {
            $sWhereExt .= "AND date(printf('%04d-%02d-%02d', per_BirthYear, per_BirthMonth, per_BirthDay)) <= '" . InputUtils::legacyFilterInput($_POST['BirthDate2'], 'char', 10) . "' ";
        } else {
    ...
    if (!empty($_POST['AnniversaryDate1'])) {
        ...
        if ($annivStart['year'] < date('Y') || ($annivStart['year'] == date('Y') && $annivStart['mon'] <= date('m') && $annivStart['mday'] <= date('d'))) {
            ...
            if ($isSqlite) {
                $sWhereExt .= "AND CAST(strftime('%j', fam_WeddingDate) AS INTEGER) >= CAST(strftime('%j', '" . InputUtils::legacyFilterInput($_POST['AnniversaryDate1'], 'char', 10) . "') AS INTEGER) ";
            } else {
    ...
    if ($_POST['AnniversaryDate2'] != date('Y-m-d')) {
        ...
        if ($annivEnd['year'] < date('Y') || ($annivEnd['year'] == date('Y') && $annivEnd['mon'] <= date('m') && $annivEnd['mday'] <= date('d'))) {
            ...
            if ($isSqlite) {
                $sWhereExt .= "AND CAST(strftime('%j', fam_WeddingDate) AS INTEGER) <= CAST(strftime('%j', '" . InputUtils::legacyFilterInput($_POST['AnniversaryDate2'], 'char', 10) . "') AS INTEGER) ";
            } else {
...
if ($sFormat === 'addtocart') {
    ...
    $rsLabelsToWrite = csvCreateFileDbQuery($sSQL);
    while ($aRow = csvCreateFileDbFetchArray($rsLabelsToWrite)) {
        extract($aRow);
    ...
    if ($sFormat === 'default') {
        while ($aRow = csvCreateFileDbFetchArray($rsCustomFields)) {
            extract($aRow);
        ...
        while ($aFamRow = csvCreateFileDbFetchArray($rsFamCustomFields)) {
            extract($aFamRow);
        ...
        if (!($bSkipIncompleteAddr && (strlen($sCity) === 0 || strlen($sState) === 0 || strlen($sZip) === 0 || (strlen($sAddress1) === 0 && strlen($sAddress2) === 0)))) {
            ...
            if (!($bSkipNoEnvelope && (strlen($fam_Envelope) === 0))) {
                ...
                if ($bUsedCustomFields && ($sFormat == 'default')) {
                    ...
                    $rsCustomData = csvCreateFileDbQuery($sSQLcustom);
                    $aCustomData = csvCreateFileDbFetchArray($rsCustomData);

                    if (csvCreateFileDbNumRows($rsCustomData) > 0) {
                        // Add custom field data
                        csvCreateFileDbDataSeek($rsCustomFields, 0);
                        while ($aCustomField = csvCreateFileDbFetchArray($rsCustomFields)) {
                            $custom_Field = '';
                            $custom_Special = '';
                            $type_ID = '';
                    ...
                    $rsFamCustomData = csvCreateFileDbQuery($sSQLFamCustom);
                    $aFamCustomData = csvCreateFileDbFetchArray($rsFamCustomData);

                    if (@csvCreateFileDbNumRows($rsFamCustomData) > 0) {
                        // Add custom field data
                        csvCreateFileDbDataSeek($rsFamCustomFields, 0);
                        while ($aFamCustomField = csvCreateFileDbFetchArray($rsFamCustomFields)) {
                            $fam_custom_Field = '';
                            $fam_custom_Special = '';
                            $type_ID = '';

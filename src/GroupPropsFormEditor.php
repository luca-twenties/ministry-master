...
function groupPropsFormDbQuery(string $sql)
{
...
function groupPropsFormDbNumRows($result): int
{
...
function groupPropsFormDbFetchArray(&$result, $mode = null)
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function groupPropsFormDbGetColumnCount(string $tableName): int
{
    global $isSqlite, $dbConnection, $cnInfoCentral;

    if ($isSqlite) {
        $stmt = $dbConnection->query('PRAGMA table_info(' . $tableName . ')');
...
$rsGroupInfo = groupPropsFormDbQuery($sSQL);
extract(groupPropsFormDbFetchArray($rsGroupInfo));
...
<div class="card card-body">
    ...
    if (isset($_POST['SaveChanges'])) {
        ...
        $rsPropList = groupPropsFormDbQuery($sSQL);
        $numRows = groupPropsFormDbNumRows($rsPropList);

        for ($row = 1; $row <= $numRows; $row++) {
            $aRow = groupPropsFormDbFetchArray($rsPropList);
            extract($aRow);
        ...
        if (isset($_POST['AddField'])) {
            ...
            if (strlen($newFieldName) === 0) {
                ...
                $rsPropNames = groupPropsFormDbQuery($sSQL);
                while ($aRow = groupPropsFormDbFetchArray($rsPropNames)) {
                ...
                if (!$bDuplicateNameError) {
                    ...
                    $rsPropList = groupPropsFormDbQuery($sSQL);
                    $newRowNum = groupPropsFormDbNumRows($rsPropList) + 1;
                    ...
                    $newFieldNum = groupPropsFormDbGetColumnCount($tableName);
                    ...
                    if ($newFieldType == 12) {
                        ...
                        $aTemp = groupPropsFormDbFetchArray(groupPropsFormDbQuery($sSQL));
                        if ($aTemp[0] > 9) {
                            $newListID = $aTemp[0] + 1;
                        } else {
                            $newListID = 10;
                    ...
                    $sSQL .= ' DEFAULT NULL ;';
                    $result = groupPropsFormDbQuery($sSQL);
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                if ($aTypeFields[$row] == 9) {
                                    ...
                                    $rsGroupList = groupPropsFormDbQuery($sSQL);

                                    while ($aRow = groupPropsFormDbFetchArray($rsGroupList)) {
                                        extract($aRow);

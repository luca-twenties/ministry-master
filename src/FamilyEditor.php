...
function familyEditorDbQuery(string $sql, bool $bStopOnError = true)
{
    global $isSqlite, $dbConnection;

    if (!$isSqlite) {
        return RunQuery($sql, $bStopOnError);
    }
...
function familyEditorDbFetchArray(&$result, ?int $mode = null)
{
    if ($mode === null) {
...
function familyEditorDbNumRows($result): int
{
    if (is_array($result) && array_key_exists('rows', $result)) {
...
function familyEditorDbDataSeek(&$result, int $offset): void
{
    if (is_array($result) && array_key_exists('rows', $result)) {
        $result['index'] = $offset;
        return;
...
$rsCustomFields = familyEditorDbQuery($sSQL);
$numCustomFields = familyEditorDbNumRows($rsCustomFields);

// Get Field Security List Matrix
$sSQL = 'SELECT * FROM list_lst WHERE lst_ID = 5 ORDER BY lst_OptionSequence';
$rsSecurityGrp = familyEditorDbQuery($sSQL);

while ($aRow = familyEditorDbFetchArray($rsSecurityGrp)) {
    extract($aRow);
...
if (isset($_POST['FamilySubmit']) || isset($_POST['FamilySubmitAndAdd'])) {
    ...
    $aCustomData = [];
    while ($rowCustomField = familyEditorDbFetchArray($rsCustomFields)) {
    ...
    if (!$bErrorFlag) {
        ...
        if ($iFamilyID < 1) {
            ...
            for ($iCount = 1; $iCount <= $iFamilyMemberRows; $iCount++) {
                if (strlen($aFirstNames[$iCount]) > 0) {
                    ...
                    $note->save();
                    if (!$isSqlite) {
                        familyEditorDbQuery('LOCK TABLES person_custom WRITE');
                    }
                    $sSQL = 'INSERT INTO person_custom (per_ID) VALUES ('
                        . $dbPersonId . ')';
                    familyEditorDbQuery($sSQL);
                    if (!$isSqlite) {
                        familyEditorDbQuery('UNLOCK TABLES');
                    }
                }
            }
            $family = FamilyQuery::create()->findPk($iFamilyID);
        ...
        if ($numCustomFields > 0) {
            $sSQL = '';
            familyEditorDbDataSeek($rsCustomFields, 0);
            ...
            if ($sSQL > '') {
                ...
                if ($isSqlite) {
                    $ensureSQL = 'INSERT OR IGNORE INTO family_custom (fam_ID) VALUES (' . $iFamilyID . ')';
                    familyEditorDbQuery($ensureSQL);
                    $updateSQL = 'UPDATE family_custom SET ' . $sSQL . ', fam_ID = ' . $iFamilyID;
                    familyEditorDbQuery($updateSQL);
                } else {
                    $sSQL = 'REPLACE INTO family_custom SET ' . $sSQL . ', fam_ID = ' . $iFamilyID;
                    //Execute the SQL
                    familyEditorDbQuery($sSQL);
                }
            }
    ...
    if ($family) {
        ...
        $rsFamily = familyEditorDbQuery($sSQL);
        extract(familyEditorDbFetchArray($rsFamily));
        ...
        $rsCustomData = familyEditorDbQuery($sSQL);
        $aCustomData = familyEditorDbFetchArray($rsCustomData);
        ...
        $rsMembers = familyEditorDbQuery($sSQL);
        $iCount = 0;
        $iFamilyMemberRows = 0;
        while ($aRow = familyEditorDbFetchArray($rsMembers)) {
            extract($aRow);
            $iCount++;
            $iFamilyMemberRows++;
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <select name="Classification<?= $iCount ?>" class="form-control form-control-sm">
                                                        ...
                                                        $rsClassifications = familyEditorDbQuery($sSQL);

                                                        //Display Classifications
                                                        while ($aRow = familyEditorDbFetchArray($rsClassifications)) {
                                                            extract($aRow);

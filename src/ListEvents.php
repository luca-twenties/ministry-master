...
function listEventsDbQuery(string $sql)
{
...
function listEventsFetchAssoc(&$result)
{
...
$sSQL = 'SELECT DISTINCT event_types.*
         ...
         ORDER BY type_id';
$rsOpps = listEventsDbQuery($sSQL);
$eventTypes = [];
while ($row = listEventsFetchAssoc($rsOpps)) {
    $eventTypes[] = $row;
}
...
foreach ($allMonths as $mVal) {
    ...
    $rsOpps = listEventsDbQuery($sSQL);
    $numRows = $rsOpps ? count($rsOpps['rows']) : 0;
    ...
    while ($row = listEventsFetchAssoc($rsOpps)) {
        ...
        $attendResult = listEventsDbQuery($attendSQL);
        $attendRow = listEventsFetchAssoc($attendResult);
        ...
        $checkedInResult = listEventsDbQuery($checkedInSQL);
        $checkedInRow = listEventsFetchAssoc($checkedInResult);
        ...
        $checkedOutResult = listEventsDbQuery($checkedOutSQL);
        $checkedOutRow = listEventsFetchAssoc($checkedOutResult);
        ...
        $countResult = listEventsDbQuery($countSQL);
        $eventCounts = [];
        while ($countRow = listEventsFetchAssoc($countResult)) {
      ...
      <table class="table table-striped table-hover mb-0">
        ...
        <tbody>
          ...
          if ($eType !== 'All' && !empty($events[0]['counts'])):
              ...
              $avgResult = listEventsDbQuery($avgSQL);
              $averages = [];
              while ($avgRow = listEventsFetchAssoc($avgResult)) {
                  $averages[] = $avgRow;
              }
              if (!empty($averages)):
                  ?>

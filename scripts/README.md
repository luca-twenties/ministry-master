...
| `doctor.php` | Run system diagnostics (CLI) | `./scripts/doctor.php` |
...
**Runs system diagnostics from the command line.**

```bash
./scripts/doctor.php\n./scripts/doctor.php --json
```

**What it checks:**
- PHP version + required extensions
- Database connectivity + required tables
- Active config path + standalone data directory

**Requires:** PHP CLI and a valid config (or standalone auto-config env vars)
